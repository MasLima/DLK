...
lcFile = "ComCenCos"
lcProceso = "Rep_ComCenCos()"
lnPag = 0
R = 0
Imprimetexto(lcFile,lcProceso)
RETURN

PROCEDURE REP_ComCenCos
LOCAL lnTotImpSol,lnTotImpDol,lnCtoImpSol,lnCtoImpDol,lnCenImpSol,lnCenImpDol
LOCAL lcCenCos,lcTipCto,lcCodCto,lcDesCto,lcDesCenCos
...
DO WHILE !EOF()
	lcCenCos = CenCos
	lcDesCenCos = ALLTRIM(DesCenCos)
	R = R + 1
	@R,001 SAY "CenCos : "+PADR(lcDesCenCos,40)
	R = R + 1
	STORE 0 TO lnCenImpSol,lnCenImpDol
	DO WHILE !EOF() AND CenCos = lcCenCos
		lcTipCto = TipCto
		lcCodCto = CodCto
		...
		DO WHILE !EOF() AND CenCos = lcCenCos AND TipCto+CodCto = lcTipCto+lcCodCto
			R = R + 1
			IF R > 55
				TIT_ComCenCos()
				R = R + 1
		...
		lnCenImpSol = lnCenImpSol + lnCtoImpSol
		lnCenImpDol = lnCenImpDol + lnCtoImpDol
		R = R + 1
	ENDDO
	R = R + 1
	@R,031 SAY "Total "+lcDesCenCos
	@R,076 SAY lnCenImpDol PICTURE "@Z 99,999,999.99"
	@R,090 SAY lnCenImpSol PICTURE "@Z 99,999,999.99"
	lnTotImpSol = lnTotImpSol + lnCenImpSol
	lnTotImpDol = lnTotImpDol + lnCenImpDol
	R = R + 1
ENDDO
R = R + 1
